...
function patientCard(patient, isAssigned) {
  ...
  const openBtn = isAssigned
    ? `<button data-action="open" data-id="${patient.user_id}" class="rounded-md bg-sky-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-sky-700">Open Dashboard</button>`
    : `<button data-action="open" data-id="${patient.user_id}" class="rounded-md bg-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-600 cursor-not-allowed" disabled>Assign to Open</button>`;
  ...
  wrapper.querySelectorAll("button").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      if (action === "open") {
        if (!assignedPatients.has(userId)) return;
