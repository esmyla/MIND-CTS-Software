"""
Wrist Flexion Onboarding Calibration Application with WebSocket Support
========================================================================
This application calibrates a new user's starting angles by measuring their
maximum range of motion in both forward and backward directions, and streams
real-time data to the Flutter app during calibration.
...
calibration_state = {
    "angle": 0.0,
    "phase": "instructions",  # "instructions", "forward", "backward", "complete"
    "attempt": 0,
    "max_attempts": NUM_ATTEMPTS,
    "is_stable": False,
    "stability_progress": 0.0,  # 0.0 to 1.0
    "recorded_angles_forward": [],
    "recorded_angles_backward": [],
    "message": "Welcome to calibration",
}
...
async def handle_websocket(websocket, path):
    """Handle WebSocket connection from Flutter app during calibration."""
    print(f"✓ Flutter app connected from {websocket.remote_address}")
    
    try:
        while not quit_requested:
            # Send current calibration state to Flutter app
            try:
                await websocket.send(json.dumps(calibration_state))
            except Exception as e:
                print(f"Error sending to Flutter: {e}")
                break
            
            # Check for commands from Flutter (if needed)
            try:
                message = await asyncio.wait_for(websocket.recv(), timeout=0.01)
                # Could handle start/stop commands here if needed
    ...
    def update(self, current_angle):
        ...
        if angle_range <= STABILIZATION_THRESHOLD:
            if not self.is_stable:
                ...
                time_held = time.time() - self.stable_start_time
                progress = min(1.0, time_held / STABILIZATION_TIME)
                calibration_state["stability_progress"] = progress
                
                if time_held >= STABILIZATION_TIME:
            ...
            calibration_state["stability_progress"] = 0.0
...
def main():
    ...
    global calibration_state, quit_requested
    ...
    calibration_state.update({
        "phase": "instructions",
        "attempt": 0,
        "message": "Press 's' to start calibration"
    })
    ...
    try:
        while True:
            ...
            # Update global state with current angle
            calibration_state["angle"] = current_angle if current_angle is not None else 0.0
            ...
            if show_instructions and time.time() - instructions_shown_time < 20.0:
                ...
                calibration_state["phase"] = current_phase
                calibration_state["attempt"] = attempt_num
                ...
                calibration_state["message"] = instruction
                ...
                if current_angle is not None:
                    if state.update(current_angle):
                        if current_phase == "forward":
                            ...
                            calibration_state["recorded_angles_forward"] = [round(a, 1) for a in forward_angles]
                            print(f"Forward attempt {attempt_num}: {state.recorded_angle:.1f}°")
                        else:
                            backward_angles.append(state.recorded_angle)
                            calibration_state["recorded_angles_backward"] = [round(a, 1) for a in backward_angles]
        ...
        if len(forward_angles) == NUM_ATTEMPTS and len(backward_angles) == NUM_ATTEMPTS:
            ...
            # Update global state for Flutter
            calibration_state.update({
                "phase": "complete",
                "message": f"Calibration complete! Forward: {calibrated_forward}°, Backward: {calibrated_backward}°",
                "final_forward": calibrated_forward,
                "final_backward": calibrated_backward
            })
